...
Link for this presentation:  http://brunaw.com/slides/rladies-dublin/intro-to-rf.html
...
- `ggplot2`: graphic tool,s following the [grammar of graphics](https://byrneslab.net/classes/biol607/readings/wickham_layered-grammar.pdf)
...
Using`diamonds` data, from the `ggplot2` package, split in **train** and **test** sets;
...
Packages for random forests in `R`, that are actually written in `c++`:
...
Common arguments: 
  ...
  - `num.trees`: number of trees to build
...
Random forests are highly sensitive to correlated predictors: it 
splits their importance 
